...
export function renderTaskResult({ text, failureMessage, reasoningSummary }, { title, jobId = null, write = false, agentName = "Hermes" } = {}) {
  const lines = [
    `# ${title ?? "Kilo task"}`,
...
export function renderReviewResult(text, { reviewLabel = "Review", targetLabel = "working tree", sessionId = null, agentName = "Hermes" } = {}) {
  const lines = [
    `# ${reviewLabel}`,
    `- Target: ${targetLabel}`,
...
export function renderStoredJobResult(job, storedJob, { agentName = "Hermes" } = {}) {
...
export function renderTransferResult({ threadId, resumeCommand, agentName = "Hermes" }) {
  return [
    `# ${agentName} transfer`,
