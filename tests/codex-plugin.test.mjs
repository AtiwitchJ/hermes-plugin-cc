...
const PLUGIN_ROOT = path.join(ROOT, "plugins", "hermes");
...
test("codex marketplace entry points at the plugin source", () => {
  ...
  assert.equal(parsed.name, "agents-plugin-cc-hermes");
  const entry = parsed.plugins.find((plugin) => plugin.name === "hermes");
  assert.ok(entry, "Missing hermes entry");
  assert.equal(entry.source.source, "local");
  assert.equal(entry.source.path, "./plugins/hermes");
