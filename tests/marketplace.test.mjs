...
test("marketplace.json exists and is valid JSON", () => {
  ...
  assert.equal(parsed.plugins[0].name, "hermes");
});
...
test("every command file exists", () => {
  ...
  const cmdDir = path.join(ROOT, "plugins", "hermes", "commands");
...
test("rescue subagent exists", () => {
  const agentFile = path.join(ROOT, "plugins", "hermes", "agents", "hermes-rescue.md");
